...
fun OnboardingScreen(caregiverId: String, onComplete: (String) -> Unit, onBack: () -> Unit, viewModel: OnboardingViewModel = hiltViewModel()) {
    ...
    Scaffold(snackbarHost = { SnackbarHost(snackbarHostState) }) { paddingValues ->
        ...
        when (step) {
            0 -> { SectionTitle("Basic details"); Field("Username (Optional)", data.username) { data = data.copy(username = it) }; Field("Full name", data.name) { data = data.copy(name = it) }; Field("Preferred name", data.preferredName) { data = data.copy(preferredName = it) }; Field("Date of birth", data.dateOfBirth) { data = data.copy(dateOfBirth = it) }; Field("Age", data.age) { data = data.copy(age = it.filter(Char::isDigit).take(3)) }; Field("Gender", data.gender) { data = data.copy(gender = it) }; Field("City", data.city) { data = data.copy(city = it) }; Field("Language (en/as/bn)", data.language) { data = data.copy(language = it) }; Field("Profile photo URL (optional)", data.profileImageUrl) { data = data.copy(profileImageUrl = it) } }
            1 -> { SectionTitle("Care and safety"); Field("Your relationship to the patient", data.caregiverRelationship) { data = data.copy(caregiverRelationship = it) }; Field("Caregiver contact", data.caregiverContact) { data = data.copy(caregiverContact = it) }; Field("Emergency contact", data.emergencyContact) { data = data.copy(emergencyContact = it) }; Text("Cognitive stage", fontWeight = FontWeight.Bold, modifier = Modifier.padding(top = 8.dp)); Row(Modifier.fillMaxWidth().horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(5.dp)) { com.nercare.cogcare.domain.model.CognitiveStage.entries.forEach { stage -> FilterChip(selected = data.stage == stage, onClick = { data = data.copy(stage = stage) }, label = { Text(stage.label) }) } }; Field("Diagnosis details (optional)", data.diagnosisDetails) { data = data.copy(diagnosisDetails = it) }; Field("Medical conditions (optional / Unknown)", data.conditions) { data = data.copy(conditions = it) }; Field("Medications (optional / Unknown)", data.medications) { data = data.copy(medications = it) }; Field("Daily independence baseline", data.independence) { data = data.copy(independence = it) } }
            2 -> { SectionTitle("Personalisation"); Text("These details power relevant conversations, games and patient matching.", color = TextSecondaryMuted); Field("Hobbies", data.hobbies) { data = data.copy(hobbies = it) }; Field("Favourite music", data.music) { data = data.copy(music = it) }; Field("Favourite foods", data.foods) { data = data.copy(foods = it) }; Field("Favourite activities", data.activities) { data = data.copy(activities = it) }; Field("Profession", data.profession) { data = data.copy(profession = it) }; Field("Important people", data.people) { data = data.copy(people = it) }; Field("Important places", data.places) { data = data.copy(places = it) }; Field("Important events", data.events) { data = data.copy(events = it) } }
                ...
                if (credentials == null) {
                    Text("A secure Username and one-time password will be generated.")
                    Button(onClick = { viewModel.createPatient(caregiverId, data) }, enabled = data.name.isNotBlank() && caregiverId.isNotBlank() && !isLoading, modifier = Modifier.fillMaxWidth().padding(top = 20.dp).height(56.dp)) { 
                    ...
                    Button(onClick = { onComplete(credentials.username) }, modifier = Modifier.fillMaxWidth().padding(top = 20.dp).height(56.dp)) { Text("Open caregiver dashboard") }
                }
            }
        }
        }
        if (step < 3) Row(Modifier.fillMaxWidth().padding(top = 24.dp), horizontalArrangement = Arrangement.SpaceBetween) { TextButton(onClick = { if (step == 0) onBack() else step-- }) { Text("Back") }; Button(onClick = { step++ }, enabled = step != 0 || data.name.isNotBlank()) { Text("Next") } }
    }
}
...
@Composable private fun Field(label: String, value: String, update: (String) -> Unit) { OutlinedTextField(value, update, label = { Text(label) }, modifier = Modifier.fillMaxWidth().padding(vertical = 5.dp), singleLine = true) }
